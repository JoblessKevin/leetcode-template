...
public class EditDistance {
    public static class BruteForce {
        ...
        private int minDistanceHelper(String word1, String word2, int i, int j) {
            // 如果 word1 已經走到盡頭，剩下的字元都要插入
            if (i == word1.length()) {
                return word2.length() - j;
            }
            // 如果 word2 已經走到盡頭，剩下的字元都要刪除
            if (j == word2.length()) {
            ...
            if (word1.charAt(i) == word2.charAt(j)) {
                ...
                // 字元不同，考慮三種操作：插入、刪除、替換
                int insertOp = minDistanceHelper(word1, word2, i, j + 1); // 插入
                int deleteOp = minDistanceHelper(word1, word2, i + 1, j); // 刪除
                int replaceOp = minDistanceHelper(word1, word2, i + 1, j + 1); // 替換
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Brute Force: " + bruteForce.minDistance(word1, word2));

        DP dp = new DP();
        System.out.println("DP: " + dp.minDistance(word1, word2));

        Optimized optimized = new Optimized();
        System.out.println("Optimized: " + optimized.minDistance(word1, word2));
    }
}
